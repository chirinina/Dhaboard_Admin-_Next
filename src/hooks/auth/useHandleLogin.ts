...
export const useHandleLogin = (onLoginSuccess?: () => void) => {
  ...
  const handleLogin = useCallback(
    async (data: LoginData, rememberMe: boolean) => {
      /** Check if running in presentation mode (no backend) — bypass login directly */
  ...
  const validationSchema = useMemo(() => {
    /** In presentation mode skip all field validation so the form submits immediately */
    if (isPresentationModeClient()) {
  ...
  }, [t]) as Yup.ObjectSchema<LoginData>;
