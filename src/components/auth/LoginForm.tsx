...
export const LoginForm = ({
  ...
  // Detect presentation mode only on the client to avoid SSR/client mismatch
  const [isDemo, setIsDemo] = useState(false);
  useEffect(() => {
    setIsDemo(isPresentationModeClient());
  }, []);

  return (
    <div className="w-full sm:max-w-84 md:w-78 1xl:w-84 flex flex-col items-center">
      <h1 className="text-4xl sm:text-3xl 1xl:text-4xl font-bold mb-4 1xl:mb-6 mt-2 1xl:mt-4 text-primaryText">
        {t("signIn")}
      </h1>
      {!isDemo && <div className="mb-8 1xl:mb-12" />}

      <form
        className="w-full flex flex-col gap-4 items-center"
        onSubmit={handleSubmit((data: LoginData) => onSubmit(data, rememberMe))}
      >
