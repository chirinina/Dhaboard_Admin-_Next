...
export const Layout = ({ children }: LayoutProps) => {
  ...
  return (
    <>
      ...
      {!isAuthPage && (
        <div className="hidden xl:block fixed top-1/2 right-0 -translate-y-1/2 z-50">
          <SettingsDrawer>
            <button
              tabIndex={0}
              className="w-12 h-14 rounded-l-xl border-y border-l border-mainBorder bg-floatingMenuButtonBg hover:bg-mainColor/10 hover:text-mainColor text-settingsIcon stroke-settingsIcon fill-settingsIcon shadow-[-4px_0_15px_rgba(0,0,0,0.05)] flex items-center justify-center transition-colors cursor-pointer"
      ...
      <div
        className={`flex min-h-screen w-full ${
          isAuthPage ? "bg-authPageBg" : "bg-secondaryBg"
      ...
      >
        ...
        <div
          className={`flex flex-col w-full ${
            isAuthPage ? "" : CONTENT_MAX_WIDTH
          } ${isAuthPage ? "" : "mx-auto"} relative`}
        >
